...
async def login(request: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
    ...
    print(f"\nIs entered password same as stored password? {checkHash(request.password, salt, user.passwd)}\n")
...
async def create_user(request: schemas.User, db: Session = Depends(get_db)):
    try:
        ...
        return {"detail": f"User with email {request.email} already exists. Please login."}
